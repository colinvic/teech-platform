...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const { data, error } = await supabase
    ...
    .select(`
      ...
      wwc_number, wwc_state, wwc_expiry, wwc_verified,
      kyc_verified, stripe_onboarding_complete,
      bio, subjects, year_levels,
      hourly_rate_cents, session_count, rating_average,
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const updateMap: UpdateMap = {
    ...
    verify_wwc:   { wwc_verified: true },
    unverify_wwc: { wwc_verified: false },
  }
  ...
  await supabase.from('compliance_audit_log').insert({
    ...
    actor_role: 'admin',
    action:     `admin.tutor.${action}`,
    table_name: 'tutor_profiles',
    record_id:  tutorId,
    metadata:   { note, updates },
  })
