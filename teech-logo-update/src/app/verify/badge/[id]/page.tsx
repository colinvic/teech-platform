...
import { Logo } from '@/components/Logo'
  IconBadge, IconCheckCircle, IconXCircle,
  IconStar, IconLightning, IconFlame, IconVerified,
...
// Pure CSS rarity config — no emojis
const RARITY_CONFIG = {
...
export default async function BadgeVerificationPage({
  ...
  const issuedDate = new Date(badge.issuedAt).toLocaleDateString('en-AU', {
  ...
  return (
    <div className="min-h-screen bg-deep flex flex-col items-center justify-center px-4 py-12">
      ...
      {/* Badge card */}
      <div
        className={`w-full max-w-sm bg-surface rounded-3xl p-8 text-center border-2 ${
          badge.isValid ? config.border : 'border-red-500/40'
        }`}
      >
        ...
        <div
          className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold uppercase tracking-widest mb-6 border ${
            badge.isValid
              ? 'bg-lime/12 text-lime border-lime/25'
        ...
        {/* Icon — CSS only, sized to feel premium */}
        <div
          className={`w-20 h-20 rounded-full border-2 flex items-center justify-center mx-auto mb-4 animate-badge-pop ${
            badge.isValid ? config.border : 'border-red-500/40'
          }`}
        >
          {badge.isValid
            ? <Icon className={`w-9 h-9 ${config.iconClass}`} />
            : <IconXCircle className="w-9 h-9 text-red-400" />
          }
        </div>

        {/* Rarity label */}
        <p className={`text-xs font-bold uppercase tracking-widest mb-2 ${config.iconClass}`}>
          {config.label}
        </p>
        ...
        <p className="text-teech-muted text-sm mb-6">
          {badge.subjectName} &middot; {yearLabel}
        </p>
        ...
        <div className="space-y-1 text-sm text-teech-muted">
          <p>
            Awarded to{' '}
            <span className="text-white font-semibold">{badge.studentDisplayName}</span>
          </p>
          <p>{issuedDate}</p>
        </div>
      </div>
      ...
      <div className="mt-8 text-center max-w-sm space-y-2">
        <div className="flex items-center justify-center gap-1.5 text-xs text-teech-muted/70">
          ...
          <span>
            Issued by <span className="text-teal">teech.au</span> and cryptographically verified.
            Badge ID and score cannot be altered.
          </span>
        </div>
        <p className="text-[10px] text-teech-muted/40 font-mono break-all">
          {badge.badgeId}
        </p>
      </div>
      ...
      <Link
        href="/"
        className="mt-8 text-xs text-teech-muted hover:text-teal transition-colors inline-flex items-center gap-1"
      >
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" strokeWidth="1.75" className="w-3.5 h-3.5" aria-hidden>
